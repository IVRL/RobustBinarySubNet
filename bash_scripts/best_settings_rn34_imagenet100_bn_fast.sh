...
train(){
    ...
    for d in 0.1; do
        for pr in 0.01; do
            ...
            model_name=$1-rn34-ours-bn-fast-$dataset
            savedir=/path/to/store/the/experiments

            python run/train.py \
                --config configs/advprune/$dataset/$model/$model-binary-weights-fast.yaml \
                --log-dir $savedir/ \
                ...
                --eval-attack name=PGD,step_size=0.00196078,threshold=2,iter_num=10 \
                --end-with-bn \
                --debug;

            checkpoint_dir=$savedir/$model-binary-weights-fast
