...
train(){
    ...
    for d in 0.1; do
        for pr in 0.01; do
            ...
            model_name=$1-best-settings-trades-ours-bn-d$d-pr$pr-$dataset
            savedir=/path/to/store/the/experiments

            python run/train.py \
                --config configs/advprune/$dataset/$model/$model-binary-weights-trades.yaml \
                --log-dir $savedir/ \
            ...
            checkpoint_dir=$savedir/$model-binary-weights-trades
