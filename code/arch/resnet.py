...
class ResNet(nn.Module):
    def __init__(self, builder, block, layers, num_classes=1000, base_width=64):
        ...
        if Args.args.last_layer_dense:
            self.fc = nn.Conv2d(512 * block.expansion, Args.args.num_classes, 1)
        else:
...
def ResNet18_imagenet100(pretrained=False):
    return ResNet(get_builder(), BasicBlock, [2, 2, 2, 2], 100)

def ResNet34_imagenet100(pretrained=False):
    return ResNet(get_builder(), BasicBlock, [3, 4, 6, 3], 100)

def ResNet50_imagenet100(pretrained=False):
    return ResNet(get_builder(), Bottleneck, [3, 4, 6, 3], 100)

def ResNet101_imagenet100(pretrained=False):
    return ResNet(get_builder(), Bottleneck, [3, 4, 23, 3], 100)
