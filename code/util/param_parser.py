...
class ArgListParser(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string = None):
        try:
            arg_list = values.lstrip("[").rstrip("]").split(",")
            for arg in arg_list:
                if len(arg.split("=")) == 2:
                    k, v = arg.split("=")
                    self.list.append(k)
            ...
            setattr(namespace, self.dest, self.list)
        except:
            raise ValueError('Failed when parsing %s as argument list' % values)
